...
public class JwtUtils {
    ...
    public static final long EXPIRED_MINUTES = 240;
    private JwtUtils(){
    ...
    private static Date toExpireDate(Date start){
        ...
        LocalDateTime endTime = dateTime.plusDays(EXPIRED_DAYS).plusHours(EXPIRED_HOURS).plusMinutes(EXPIRED_MINUTES);
    ...
    public static boolean isTokenValid(String token){
        ...
        try{
            Jwts.parserBuilder()
                    .setSigningKey(generateKey()).build()
                    .parseClaimsJws(refactorToken(token)).getBody(); //processar e validar um JWT assinado.
